    ...
    for (let i = 0; i < airportInfo.length; i++) {
        ...
        if (i === 0) {
            ...
            goButton.addEventListener('click', async function () {
                try {
                    const response = await fetch(`${apiUrl}update?icao=${airportInfo[i].ident}&game_id=${gameId}&points=${alljason.points}&money=${alljason.money}`);
                    if (!response.ok) {
                        throw new Error(response.status.toString());
                    }
            ...
            goButton.addEventListener('click', async function () {
                try {
                    const response = await fetch(`${apiUrl}update?icao=${airportInfo[i].ident}&game_id=${gameId}&points=20000&money=3000`);
                    if (!response.ok) {
                        throw new Error(response.status.toString());
                    }
                    const data = await response.json();
